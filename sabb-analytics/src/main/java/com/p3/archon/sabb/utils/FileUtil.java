...
public class FileUtil {
    ...
    public static void checkCreateDirectory(String fileDir) throws Exception {
        if (!checkForDirectory(fileDir)) {
           // LOGGER.debug("CreateDirectory = " + fileDir);
            createDir(fileDir);
        }
    }
    ...
    public static File createDir(String dir) throws IOException {
        ...
        if (!tmpDir.exists()) {
            ...
            // LOGGER.debug("Not creating directory, " + dir + ", this directory
            // already exists.");
        }
        return tmpDir;
    }
    ...
    public static boolean movefile(String filetoMove, String destinationFilePath, boolean haltIfFail) {
        ...
        if (!success) {
            LOGGER.error("The file " + filetoMove + " was not successfully moved");
            if (haltIfFail)
                System.exit(1);
        }
        return success;
    ...
    public static boolean deleteDirectory(File dir) {
        ...
        if (dir.isDirectory()) {
            ...
            for (String element : children) {
                ...
                if (!success) {
                    LOGGER.error("Unable to delete file: " + new File(dir, element));
                    return false;
                }
            }
        }
    ...
    public static void moveXMLFile(String file, File xmlsPath) {
        LOGGER.debug("Saving file " + file + " in " + xmlsPath.toString());
